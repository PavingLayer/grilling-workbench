...
function render() {
  ...
  app.innerHTML = `<div class="workspace">
    ...
    <article class="question-sheet" id="question" aria-labelledby="question-title"><div class="question-meta"><button class="button small question-selector" id="question-selector" data-action="questions" aria-haspopup="dialog" aria-controls="review-dialog">Question ${index + 1} of ${state.questionnaire.questions.length} <span aria-hidden="true">▾</span></button><span id="question-badges"></span></div><h2 id="question-title" tabindex="-1">${escape(q.title)}</h2><p class="question-context">${escape(q.context)}</p>
    ...
    <div class="question-actions"><button class="button" data-action="defer">${draft.deferred ? 'Resume question' : 'Answer later'}</button>${hasAnswer(draft) ? '<button class="button" data-action="clear">Clear answer</button>' : ''}</div></article></main>
    </div><footer class="footer-bar"><div class="footer-inner"><div class="footer-summary"><span id="footer-status" class="footer-copy"></span><button id="history-button" class="text-button" data-action="history" hidden></button></div><nav class="footer-buttons ${index === state.questionnaire.questions.length - 1 ? 'last-question' : ''}" aria-label="Question navigation"><button class="button previous-question" ${index > 0 ? `data-go="${escape(state.questionnaire.questions[index - 1].id)}"` : 'disabled'} aria-label="Previous question"><span aria-hidden="true">←</span></button><button class="button ${index === state.questionnaire.questions.length - 1 ? 'primary' : ''}" id="open-review" data-action="review">Review answers</button>${index < state.questionnaire.questions.length - 1 ? `<button class="button primary" data-go="${escape(state.questionnaire.questions[index + 1].id)}">Next question <span aria-hidden="true">→</span></button>` : ''}</nav></div></div></footer>`;
  renderChrome();
